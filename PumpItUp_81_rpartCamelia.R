...
setwd("C:/Documents and Settings/pedroc.FUERTEVENTURA/Escritorio/pedroc")
...
#lolagpnona <- as.data.frame(impute(as.matrix(lonlatgps), lmFun = "lassoR", cFun="rpartC", verbose=TRUE)$imp)
...
#nonanona       <- amelia(nonatmp), lmFun="lassoR", verbose=FALSE)

nonaend        <- as.data.frame(nonatmp)
...
#library(imputeR)
perpub$permit <- as.factor(perpub$permit)
perpub$public_meeting <- as.factor(perpub$public_meeting)
noms <- c("permit", "public_meeting")
perpubnona <- amelia(perpub[, 1:2], noms = noms) 

datIncl$permit <- as.integer(perpubnona$imputations$imp5$permit)
datIncl$public_meeting <- as.integer(perpubnona$imputations$imp5$public_meeting)
