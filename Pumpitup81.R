...
datIncl$fedistGeo <- distGeo(as.matrix(datIncl[,c('longitude','latitude')]), c(0,0))
...
rm(lolagpnona, lonlatgps, tmpVar)

#To include new variables: num_private - lga
datIncl$num_private <- datIn$num_private
datIncl$lga <- as.numeric(as.factor(datIn$lga))
#datIncl <- datIncl[,c(1:35, 37:38, 36)] #reorder
#datIncl <- datIncl[,c(1:41, 43:44, 42)] #reorder
tmpVar <- datIncl$festatus
datIncl <- datIncl[, -c(which(names(datIncl)=='festatus'))]
datIncl$festatus <- tmpVar
...
write.table(datSuboend, file=paste("Res_xxxx_", modtype,"_",numvars,"_samp100_noban_n",samp,"_Acc_", scortmp,"_",timval,".csv",sep=""),sep=","
